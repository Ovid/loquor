...
| Re-planning over literal intent | F-H, F-Q, F3-escalated, F8-escalated | Prompt carries 1500 chars of recent game output; the model infers "what the player must want next" from a stale world-model instead of translating. Corrupts valid **English** input too. |
...
error); a coverage report lists vocab nouns with no entry per language.
...
room+inventory scope (scene tracker, §8); (2) else emit the shared English
dictionary word (`key`) and let the Z-parser disambiguate.
...
3. **Map:** leading verb via core lexicon; nouns via game lexicon **or
   directly via the English vocab's dictionary words** (which makes English
   input deterministic too — `open trap door` matches vocab and passes through
   untouched); preposition via core lexicon.
...
- **Prompt rewrite:** "Translate the player's literal imperative into one
  ...
  player 'should' do next." **Drop the `recentOutput` block entirely** (it is
  the fuel for re-planning, F-H/F-Q); keep location + scope hint + antecedent.
...
- **Emit form per noun** (fixes F-Z): the shortest parser-accepted name —
  ...
  uniqueness within the game (`pump`, `brass lantern`, `trap door`). Grammar,
  prompt, lexicon resolution, and pronoun substitution all emit this form.
  Stored as a new `emit` field on `NounEntry`; `canonical` remains the stable
  lexicon key.
...
it, not have queued moves eaten by it.
