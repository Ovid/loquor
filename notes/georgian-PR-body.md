...
Adds **Georgian (ქართული)** as a selectable language: picking it translates all **Zork I** on-screen text to Georgian via the existing `src/translate/` display overlay (corpus-only, **no LLM fallback**), behind a visible **(beta)** marker, while the player types commands in **English** (Phase 1 = read-Georgian / type-English; Georgian *input* is Phase 2).
...
- **Option A wiring** (`Terminal`/picker): Georgian translates *output* while the command field raw-sends English; the model-download modal and "✦ improve" affordance are suppressed; fr/de/es behaviour is byte-for-byte unchanged (regression-pinned).
