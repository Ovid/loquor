...
- `nimm alles` → `take large bag` → "Du siehst den großen Sack hier nirgends!" The "all/everything"
  quantifier **"alles" maps to "large bag"** (the thief's Sack), so _take all_ is broken in German.
...
- **Spanish: BROKEN.** `dar cuerda al canario` → `give rope to canary` (the idiom _dar cuerda a X_
...
- **Same blind spot as Spanish/French** (uat.md: es "What do you want to put the torch in?" →
  garbled "¿Qué quieres poner la cera?"). German's LLM fallback is _less_ garbled but it's still a
...
- The bauble: floor "Messingkugel", input mis-maps (F20: Messingkugel→machine, Kugel→kitchen table),
  ...
  is likely the _intended_ German noun — worth testing `nimm das Schmuckstück` as the F20 fix.
...
- **F12, F20, F22, F24, F28** — missing noun surface forms in `de.zork1.ts`.
  ...
  oddity in F22/F28, which was just the _object_ missing → whole-clause LLM).
