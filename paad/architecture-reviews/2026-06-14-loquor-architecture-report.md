...
- **Found by:** Integration & Data
- **Status:** Fixed
- **Status reason:** Closed the "degrades **silently**" core of the finding without inventing a schema/version framework the unversioned GlkOte protocol can't supply. `reduce()` shape-sniffs content entries (`lines[]` ⇒ grid, `text[]` ⇒ buffer); an entry matching neither used to be dropped invisibly, so an upstream shape change in the SHA-pinned vendored `glkapi.js` would silently lose output with no signal. The reducer now hits an explicit `else` branch that `log.warn`s `[glk] unrecognized content entry shape (glkapi drift?)` with the entry's keys — the one place a drift is observable — then still ignores the entry best-effort so a stray/unknown window can never corrupt or wipe the transcript. Routes through the F-16 logger, so a drift now also lands in the durable ring (`window.loquorLogs()`), not just the console. Red/green: the F-10 safety-net test now asserts both the warn fires and the reduction output is unchanged; logging is a diagnostic, not a state mutation, so the reducer's "is pure — does not mutate" guarantee (and its test) still holds. **Deliberately scoped out (ponytail):** the `as any` field reads in `bridge.ts:97-109` are GlkOte _input-request_ shapes (`i.type`/`req.id`) — a stable, well-defined part of the contract, not the drift-prone content-window sniff F-10 is about; re-typing them is churn without the silent-degradation risk, so they're left as-is. Full suite green (777), typecheck green.
- **Status date:** 2026-06-14 08:29 UTC
- **Status commit:** (this commit)
