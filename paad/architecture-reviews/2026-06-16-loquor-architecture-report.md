...
- **Impact:** Medium
- **Explanation:** The output corpus registry holds only Zork I, while the input lexicon registry holds Zork I/II/III, so a non-EN player of Zork II/III gets natural-language _input_ in their language but English _output_ — a felt asymmetry between the two halves of the multilingual feature.
...
- **Impact:** Medium
- **Explanation:** The model-download path has abort/cancel but no timeout and no auto-retry; a stalled fetch (no progress) sits in `phase:'downloading'` indefinitely, leaving manual cancel as the player's only recourse. (The lazy in-`generateRaw` load path _does_ use a watchdog; the modal path does not.)
...
2. On a translator failure for a single non-English command (F2), what _should_ the player see — the untranslated line sent to the parser, a localized "couldn't translate" notice, or nothing sent? What does the spec intend?
