...
"thinking" indicator and `nl-status` notice render _inside_ the same scroll
...
- **[S4] `GEORGIAN` check accepts one Georgian char in an otherwise-English line** — `composed-lines.test.ts:39,133`. Structurally too weak to be `ka`'s only safety net; today only the _documented, accepted_ `{raw}`-echo lines (WHICH-PRINT, can't-see-any) exercise the English-mixed case. Fix: after stripping per-family-allowed echo tokens, assert no `[A-Za-z]{2,}` run remains. (conf 65, latent)
